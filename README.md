...
一个适用于 Minecraft Forge 1.20.1 的轻量级日志过滤模组，旨在减少控制台和日志文件中的垃圾信息，让开发者和玩家能更专注于关键错误，并节省硬盘空间。

## ✨ 特性

- **正则表达式过滤**：支持强大的正则表达式匹配，精准过滤特定格式的日志。
- **完全匹配过滤**：可以过滤掉指定的整条文本信息。
...
3. 启动游戏一次。模组会自动生成默认配置文件 `config/logfilter-common.toml`。
...
```toml
filterRules = [
    ".*Exception.*",          # 过滤所有包含 "Exception" 的日志
    ".*stack trace.*",        # 过滤堆栈跟踪
    ".*Could not pass event.*"# 过滤特定事件报错
]
```
...
```toml
loggerNames = [
    "net.minecraft.server.MinecraftServer",  # 过滤主服务器的日志
    "com.some.noisy.mod"                     # 过滤某个吵闹的模组的所有日志
]
```
...
## 💡 使用示例
### 示例 1：过滤 "Failed to verify authentication" 报错
如果你启动游戏时看到大量的 `Failed to verify authentication` 错误（通常是离线启动器或第三方登录器的问题），可以这样配置：
在 `FilterRules` 下的 `filterRules` 中添加：
```toml
filterRules = [
...
### 示例 2：屏蔽特定模组的 DEBUG 日志
假设你想屏蔽 `net.minecraft` 和 `net.minecraftforge` 的 DEBUG 级别日志：
```toml
logLevels = [
    "DEBUG",
    "TRACE"
...
```
*注意：这会屏蔽所有模组的 DEBUG 日志。如果只想屏蔽特定模组的 DEBUG 日志，请使用正则配合 loggerName，或者等待模组更新支持更细粒度的配置。*

### 示例 3：测试规则 (Debug 模式)
当你不确定写的规则是否生效时，开启 `debugMode`：
```toml
General {
    enableFilter=true
    debugMode=true  # 开启调试
...
```
开启后，每当你过滤一条日志，控制台都会显示类似：`[LogFilter] Filtered by pattern: .*Failed to verify authentication.*`
...
## ⚠️ 免责声明
使用此模组过滤日志可能会导致某些调试困难。如果遇到游戏崩溃或严重问题，建议先暂时禁用此模组或开启调试模式，以便查看完整的错误堆栈。
---
...
4.  **拦截与决策 (`FilteringLogFilter.filter` 方法)**:
...
# 解释：.* 表示任意字符，这里匹配包含 "could not find sampler named" 的所有日志
value = ".*could not find sampler named.*"
```
...
**配置代码**:
```toml
[[FilterRules.exactMatches]]
# 必须和日志里的文字一模一样（通常不包括时间戳和线程名，只包括冒号后面的内容）
value = "Shader rendertype_entity_translucent_emissive could not find sampler named Sampler2 in the specified shader program."
```
...
**配置代码**:
```toml
[[FilterRules.loggerNames]]
# 只要日志来源是 net.minecraft.client.renderer.ShaderInstance，全部屏蔽
value = "net.minecraft.client.renderer.ShaderInstance"
```
##### 方案 D：屏蔽所有 WARN 级别的日志 (使用 `logLevels`)
**适用场景**：**极度不推荐**。这会屏蔽所有模组的警告信息，可能导致你错过真正需要关注的潜在问题。
**配置代码**:
```toml
[[FilterRules.logLevels]]
value = "WARN"
```
### 总结建议
针对你这条 Shader 日志，**方案 A (`filterRules`)** 是最合理的。
因为 Shader 报错通常是模组之间资源包冲突或光影包问题导致的，往往是无害的警告。使用 `.*could not find sampler named.*` 可以屏蔽掉所有找不到采样器的警告，既解决了刷屏问题，又保留了该类可能发出的其他重要日志（比如致命错误）。
