...
https://modrinth.com/mod/log-filter1
...
7.  **拦截与决策 (`FilteringLogFilter.filter` 方法)**:
    每当 Minecraft 试图输出一行日志时，Log4j 会调用我们的 `filter(LogEvent event)` 方法。为了防止日志过滤导致游戏卡顿，我们采用了“快速失败”设计，逻辑优先级如下：
    *   **步骤 A：提取关键信息**。直接从 Log4j 的 `LogEvent` 中提取 Logger 名称、等级等关键信息。
    *   **步骤 B：快速检查（等级与 Logger）**。优先检查日志等级 (`logLevels`) 和 Logger 名称 (`loggerNames`)。这是一项极低开销的操作。如果匹配，直接丢弃日志，**完全跳过后续昂贵的消息格式化和正则匹配**，极大降低主线程开销。
    *   **步骤 C：消息格式化**。只有当日志通过了快速检查未被拦截时，才会调用 `getFormattedMessage()` 生成消息文本。
    *   **步骤 D：缓存检查**。计算日志内容的哈希值，看缓存里有没有这条日志。如果有，直接返回 `DENY`（拒绝），避免重复计算。
    *   **步骤 E：白名单检查 (`excludePatterns`)**。如果日志匹配了白名单正则，返回 `NEUTRAL`（中立/放行）。白名单优先级最高，用于保护重要日志。
